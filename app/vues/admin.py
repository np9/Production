...
# Classe de la sous catégorie de l'utilisateur pour les infos à propos de
# la personne
class VueUtilisateurContact(VueModele):
    ...
    # Colonnes invisible
    column_exclude_list = ['_mdp', 'civilite', 'confirmation',
                           'notification_email', 'notification_sms', 'inscription']
...
# Classe de la sous catégorie de l'utilisateur pour les infos
# administratives liées à l'utilisateur
class VueUtilisateurCompte(VueModele):
    ...
    # Colonnes pour filtrer
    column_filters = ['confirmation', 'notification_email',
                      'notification_sms', 'inscription']


# Classe de la sous catégorie du conducteur pour les infos à propos de la
# personne
class VueConducteurContact(VueGeo):
    ...
    colnum_list = ['civilite', 'nom', 'prenom',
                   'telephone', 'email', 'adresse', 'categorie']

    # Colonnes pour chercher
    column_searchable_list = ['nom', 'prenom', 'telephone']

    # colonne à rentrer pour ajout d'un conducteur
    form_columns = ['telephone', 'civilite', 'email', 'date_naissance', 'fax', 'prenom',
                    'nom', 'statut', 'station', 'station_entree', 'position', 'adresse', 'inscription']
...
class VueConducteurSituation(VueGeo):
    ...
    # Colonnes à afficher
    column_list = ['civilite', 'nom', 'prenom', 'telephone',
                   'situation', 'station', 'station_entree', 'position']
...
class VueVehicule(VueModele):
    ...
    form_columns = ['immatriculation', 'conducteur', 'places', 'couleur', 'marque', 'animaux', 'modele', 'american_express',
                    'carte_bleue', 'cheque', 'anglais', 'espagnol', 'allemand', 'vip', 'attelage', 'vbreak', 'voiture_basse', 'blacklist', 'mineur']
...
class VueAdresse(VueGeo):
    ...
    form_columns = ['identifiant', 'nom_rue',
                    'numero', 'cp', 'ville', 'position', 'secteur']
...
class VueFacture(VueModele):
    ...
    form_columns = ['course', 'forfait',
                    'estimation', 'montant', 'rabais', 'paiement']
...
class VueCourse(VueModele):
    ...
    form_columns = ['numero', 'trouvee', 'finie', 'utilisateur', 'conducteur', 'places', 'priorite',
                    'debut', 'fin', 'retour', 'commentaire', 'bagages', 'animaux', 'gare', 'aeroport', 'depart', 'arrivee']
...
def carte_refresh():
    req = db.session.execute(
        'SELECT position FROM Conducteurs WHERE nom = Ayadi')
